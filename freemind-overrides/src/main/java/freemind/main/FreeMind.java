...
public class FreeMind extends JFrame implements FreeMindMain {
    ...
    public FreeMind(Properties pDefaultPreferences, Properties pUserPreferences, File pAutoPropertiesFile) {
        super("FreeMind");
        // Focus searcher
        // [2026-02-11] Stop setting SecurityManager (deprecated since Java 17)
    ...
    private void updateLookAndFeel() {
        ...
        try {
            ...
            if (lookAndFeel.equals("windows")) {
                ...
                // default.
                logger.info("Default (System) Look & Feel: " + UIManager.getSystemLookAndFeelClassName());
    ...
    /**
     * Open url in WWW browser. This method hides some differences between operating
     * systems.
     */
    @Override
        ...
        if (osName.substring(0, 3).equals("Win")) {
            ...
            try {
                ...
                if (url.getProtocol().equals("file")) {
                    ...
                    if (!Tools.isBelowJava6()) {
                        ...
                        Object desktopObject = getDesktopMethod.invoke(null, new Object[] {});
                        Method openMethod = desktopObject.getClass().getMethod("open", new Class[] { File.class });
            ...
            try {
                ...
                if ("file".equals(url.getProtocol())) {
                    ...
                    final File file = Tools.urlToFile(url);
                    String[] command = { getProperty("default_browser_command_mac_open"),
    ...
    /**
     * @param  cmd         precondition: the command can be split by spaces and the
     *                     last argument is the only one that contains unicode
     *                     chars. Moreover, we are under Windows. THIS METHOD
     *                     DOESN'T SEEM TO WORK for UNICODE ARGUMENTS.
     * @throws IOException
     */
    private void execWindows(String pCommand) throws IOException {
        ...
        logger.info("Starting command array " + Arrays.toString(cmd) + ", and env for " + envName + " = "
                + HtmlTools.unicodeToHTMLUnicodeEntity((String) newEnv.get(envName), true));
    ...
    public Logger getLogger(String forClass) {
        ...
        if (mFileHandler == null && !mFileHandlerError) {
            ...
            try {
                mFileHandler = new FileHandler(getFreemindDirectory() + File.separator + LOG_FILE_NAME, 1400000, 5,
                        false);
    ...
    public static void main(final String[] args, Properties pDefaultPreferences, Properties pUserPreferences,
    ...
    private void setupProxy() {
        ...
        if ("true".equals(props.getProperty(PROXY_USE_SETTINGS))) {
            if ("true".equals(props.getProperty(PROXY_IS_AUTHENTICATED))) {
                Authenticator.setDefault(new ProxyAuthenticator(props.getProperty(PROXY_USER),
                        Tools.decompress(props.getProperty(PROXY_PASSWORD))));
            }
    ...
    private void checkForAnotherInstance(String[] pArgs) {
        ...
        if (portFile != null && new File(portFile).exists()) {
            try {
                ...
                // ERROR
                logger.info("An error occurred" + " while connecting to the FreeMind server instance."
    ...
    private void setScreenBounds() {
        ...
        addWindowListener(new WindowAdapter() {
            ...
            /*
             * fc, 14.3.2008: Completely removed, as it damaged the focus if for example the
             * note window was active.
             */
    ...
    private void loadMaps(final String[] args, ModeController pModeController, FeedBack pFeedBack) {
        ...
        if (!fileLoaded) {
            ...
            if (Tools.isPreferenceTrue(getProperty(FreeMindCommon.LOAD_LAST_MAP)) && restoreable != null
        ...
        if (!fileLoaded && Tools.isPreferenceTrue(getProperty(FreeMindCommon.LOAD_NEW_MAP))) {
            /*
             * nothing loaded so far. Perhaps, we should display a new map... According to
     ...
     * <p>
     * More than one file can be opened during startup. The filenames are stored in
     * numbered properties, i.e.
     * <ul>
    ...
    public JSplitPane insertComponentIntoSplitPane(JComponent pMindMapComponent) {
        ...
        /*
         * This means that the mind map area gets all the space that results from
         * resizing the window.
         */
