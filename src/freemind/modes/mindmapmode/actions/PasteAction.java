...
public class PasteAction extends AbstractAction implements ActorXml {
    ...
    public PasteAction(MindMapController pMindMapController) {
        super(pMindMapController.getText("paste"),
                new ImageIcon(pMindMapController.getResource("images/editpaste.png")));
        ...
        this.mMindMapController.getActionFactory().registerActor(mUndoPasteHandler, UndoPasteNodeAction.class);
    ...
    public void act(XmlAction action) {
        ...
        _paste(getTransferable(pasteAction.getTransferableContent()),
                mMindMapController.getNodeFromID(pasteAction.getNode()), pasteAction.getAsSibling(),
     ...
     * @param asSibling if true, the content is added beside the target, otherwise
     *                  as new children
    ...
    public boolean paste(Transferable t, MindMapNode target, boolean asSibling, boolean isLeft) {
        ...
        // Undo-action
        /*
         * how to construct the undo action for a complex paste? a) Paste pastes a
         * number of new nodes that are adjacent. This number should be determined.
         * 
         * 
         * d) But, as there are many possibilities which data flavor is pasted, it has
         * to be determined before, which one will be taken.
         */
        return mMindMapController.doTransaction("paste", new ActionPair(pasteAction, undoAction));
    }
    ...
    private interface DataFlavorHandler {
        ...
        void paste(Object TransferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t)
    ...
    private class FileListFlavorHandler implements DataFlavorHandler {
        ...
        public void paste(Object TransferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t) {
            ...
            for (ListIterator it = fileList.listIterator(); it.hasNext();) {
                ...
                MindMapNode node = mMindMapController.newNode(file.getName(), target.getMap());
                node.setLeft(isLeft);
    ...
    private class MindMapNodesFlavorHandler implements DataFlavorHandler {
        ...
        public void paste(Object TransferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t) {
            ...
            if (textFromClipboard != null) {
                ...
                // and now? paste it:
                String mapContent = MindMapMapModel.MAP_INITIAL_START + FreeMind.XML_VERSION
                        + "\"><node TEXT=\"DUMMY\">";
                ...
                try {
                    MindMapNode node = mMindMapController.getMindMapMapModel()
                            .loadTree(new MindMapMapModel.StringReaderCreator(mapContent), false);
                    ...
                    for (ListIterator i = node.childrenUnfolded(); i.hasNext();) {
                        MindMapNodeModel importNode = (MindMapNodeModel) i.next();
                        mMindMapController.invokeHooksRecursively(importNode, mMindMapController.getModel());
                    }
                    for (ListIterator i = node.childrenUnfolded(); i.hasNext();) {
                        MindMapNodeModel importNode = (MindMapNodeModel) i.next();
    ...
    private class DirectHtmlFlavorHandler implements DataFlavorHandler {
        ...
        public void paste(Object transferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t)
            ...
            textFromClipboard = textFromClipboard.replaceFirst("(?i)(?s)<head>.*</head>", "")
                    .replaceFirst("(?i)(?s)^.*<html[^>]*>", "<html>").replaceFirst("(?i)(?s)<body [^>]*>", "<body>")
                    .replaceAll("(?i)(?s)<script.*?>.*?</script>", "").replaceAll("(?i)(?s)</?tbody.*?>", ""). // Java
                                                                                                               // HTML
                                                                                                               // Editor
                    // does not like
                    // the tag.
            ...
            if (Tools.safeEquals(mMindMapController.getFrame().getProperty("cut_out_pictures_when_pasting_html"),
                    "true")) {
            ...
            MindMapNode node = mMindMapController.newNode(textFromClipboard, mMindMapController.getMap());
            ...
            for (int i = 1; i < links.length; i++) {
                    ...
                    try {
                        ...
                        if (referenceURL == null && !baseUrlCanceled) {
                            String referenceURLString = JOptionPane.showInputDialog(
                                    mMindMapController.getView().getSelected(),
                ...
                if (linkURL != null) {
                    ...
                    if (linkParentNode == null) {
                        linkParentNode = mMindMapController.newNode("Links", target.getMap());
                    ...
                    MindMapNode linkNode = mMindMapController.newNode(text, target.getMap());
    ...
    private class ImageFlavorHandler implements DataFlavorHandler {
        ...
        public void paste(Object transferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t)
            ...
            /*
             * BufferedImage img = null; try { img = ImageIO.read(new File("image.jpg")); }
             * catch (IOException e) { }
             */
            ...
            MindMapNode node = mMindMapController.newNode(strText, mMindMapController.getMap());
            // if only one <a>...</a> element found, set link
    ...
    private void _paste(Transferable t, MindMapNode target, boolean asSibling, boolean isLeft) {
        ...
        /*
         * DataFlavor[] fl = t.getTransferDataFlavors(); for (int i = 0; i < fl.length;
         * i++) { System.out.println(fl[i]); }
         */
    ...
    private DataFlavorHandler[] getFlavorHandlers() {
        DataFlavorHandler[] dataFlavorHandlerList = new DataFlavorHandler[] { new FileListFlavorHandler(),
    ...
    public MindMapNodeModel pasteXMLWithoutRedisplay(String pasted, MindMapNode target, boolean asSibling,
            boolean changeSide, boolean isLeft, HashMap pIDToTarget) throws XMLParseException {
        ...
        try {
            ...
            mMindMapController.invokeHooksRecursively(node, mMindMapController.getModel());
    ...
    private void insertNodeInto(MindMapNodeModel node, MindMapNode target, boolean asSibling, boolean isLeft,
            boolean changeSide) {
        MindMapNode parent;
        if (asSibling) {
     ...
     * 
     * Split the text into lines; determine the new tree structure by the number of
     * leading spaces in lines. In case that trimmed line starts with protocol
     * (http:, https:, ftp:), create a link with the same content.
     * 
     ...
     * @param isLeft TODO
     */
    private MindMapNode pasteStringWithoutRedisplay(Transferable t, MindMapNode parent, boolean asSibling,
            boolean isLeft) throws UnsupportedFlavorException, IOException {
        ...
        if (asSibling) {
            ...
            realParent = parent;
            parent = new MindMapNodeModel(mMindMapController.getFrame(), mMindMapController.getMap());
        }
        ...
        try {
            ...
            if (t.isDataFlavorSupported(MindMapNodesSelection.mindMapNodesFlavor)) {
                ...
                if (pUndoAction != null && !amountAlreadySet) {
                    pUndoAction.setNodeAmount(
                            Tools.countOccurrences(textFromClipboard, ControllerAdapter.NODESEPARATOR) + 1);
            ...
            if (t.isDataFlavorSupported(MindMapNodesSelection.fileListFlavor)) {
                /*
                 * Since the JAXB-generated interface TransferableContent doesn't supply a
                 * setTranserableAsFileList method, we have to get the fileList, clear it, and
                ...
                try {
                    ...
                    if (mindmapFile == null) {
                        JOptionPane.showMessageDialog(mMindMapController.getView(),
                                mMindMapController.getText("map_not_saved"), "FreeMind", JOptionPane.ERROR_MESSAGE);
                        return null;
                    }
                    File tempFile = File.createTempFile(
                            mindmapFile.getName().replace(FreeMindCommon.FREEMIND_FILE_EXTENSION, "_"), ".jpeg",
    ...
    protected int determineAmountOfNewNodes(Transferable t) throws UnsupportedFlavorException, IOException {
        ...
        MindMapNodeModel parent = new MindMapNodeModel(mMindMapController.getFrame(), mMindMapController.getMap());
    ...
    private Transferable getTransferable(TransferableContent trans) {
        ...
        Transferable copy = new MindMapNodesSelection(trans.getTransferable(), trans.getTransferableAsImage(),
                trans.getTransferableAsPlainText(), trans.getTransferableAsRTF(), trans.getTransferableAsHtml(),
