    ...
    public void startupMapHook() {
        ...
        if (selected == null || selected.size() != 2) {
            JOptionPane.showMessageDialog(getController().getFrame().getJFrame(),
                    "Preconditions not met: get two nodes selected", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (node1.getLink() != null || node2.getLink() != null) {
            int rc = JOptionPane.showConfirmDialog(getController().getFrame().getJFrame(),
                    "Overwrite existing link(s) in nodes?", "Confirm", JOptionPane.OK_CANCEL_OPTION);
